...
async function owns(userId, folderId) {
...
router.get('/', requireAuth, async (req, res) => {
  ...
  const rows = parentId
    ? await db.all('SELECT * FROM folders WHERE user_id = ? AND parent_id = ? ORDER BY name', [userId, parentId])
    : await db.all('SELECT * FROM folders WHERE user_id = ? AND parent_id IS NULL ORDER BY name', [userId]);
...
router.post('/', requireAuth, async (req, res) => {
  ...
  if (!(await owns(userId, parentId))) return res.status(403).json({ error: 'forbidden_parent' });

  const folder = { id: randomUUID(), user_id: userId, name, parent_id: parentId, created_at: new Date().toISOString() };
  await db.run('INSERT INTO folders (id, user_id, name, parent_id, created_at) VALUES (?, ?, ?, ?, ?)',
...
router.patch('/:id', requireAuth, async (req, res) => {
  const userId = req.user.id;
  const folder = await db.get('SELECT * FROM folders WHERE id = ? AND user_id = ?', [req.params.id, userId]);
  ...
  await db.run(`UPDATE folders SET ${sets.join(', ')} WHERE id = @id AND user_id = @userId`, params);
  const updated = await db.get('SELECT * FROM folders WHERE id = ?', [folder.id]);
...
router.delete('/:id', requireAuth, async (req, res) => {
  ...
  const folder = await db.get('SELECT * FROM folders WHERE id = ? AND user_id = ?', [req.params.id, userId]);
  if (!folder) return res.status(404).json({ error: 'not_found' });
  await db.run('DELETE FROM folders WHERE id = ? AND user_id = ?', [folder.id, userId]);
  res.json({ ok: true });
});
